> このリポジトリは、友達とのリモート交換を追加したフォークです。インストールには[このフォークのリリース](https://github.com/sacrezm/PokeTokenBar/releases/latest)をご利用ください。v2.6.2から起動時と1時間ごとに更新を確認し、**Update & Restart**でアプリ内からダウンロード・検証・インストール・再起動できます。ポケモンとトレーナーのデータは保持されます。v2.6.1以前では、この機能を有効にするため一度だけ手動インストールが必要です。詳細は[英語版のフォーク案内](README.md#about-this-fork)をご覧ください。以下は元のプロジェクトの案内です。
