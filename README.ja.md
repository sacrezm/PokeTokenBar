> このリポジトリは友達とのリモート交換を追加したフォークです。v2.6.3では更新開始時のエラーを修正し、**Update & Restart**の1回のクリックでダウンロード・検証・インストール・再起動します。ポケモンとトレーナーのデータは保持されます。v2.6.2以前のユーザーは[最新リリース](https://github.com/sacrezm/PokeTokenBar/releases/latest)を一度だけ手動でインストールしてください。詳細は[英語版のフォーク案内](README.md#about-this-fork)をご覧ください。以下は元のプロジェクトの案内です。
